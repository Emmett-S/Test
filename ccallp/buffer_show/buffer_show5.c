...
//由于buffer_show4失败，尝试将初始化python解释器的部分放在main函数中，避免多次初始化,成功显示图像序列
//问题：替换dom时，需要把所有功能写在函数中
//获取文件夹下指定后缀的文件名
...
char* BufferShow(char *YUVPath, int i)
{
...
int main(int argc,char **argv)
{
    ...
    PyObject *pModule = PyImport_ImportModule("buffer_show5");
    printf("size%ld\n",sizeof(pModule));
    if ( !pModule ) {  
    ...
    if ( !pFuncDisp || !PyCallable_Check(pFuncDisp) ) {  
        ...
        printf("find function [array2dispaly]成功!\n");
    }
    ...
    for(int i=0;i<FileNum;i++)
    {
        ...
        char* ret1 = BufferShow((char*)FileNames[i],i);
