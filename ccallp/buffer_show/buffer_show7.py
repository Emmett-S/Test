...
def from_NV12(yuv_data, frames,
              ...
              Y_WIDTH, Y_HEIGHT, Y_SIZE,
              U_V_WIDTH, U_V_HEIGHT, U_V_SIZE):
...
#传入元组时好像需要将参数命名为tuple
def array2dispaly(tuple):
    
    tuple0 = tuple[0]
    #yuv_bytes = tuple0
    yuv_bytes = base64.b64decode(tuple0)
    print("解码后：",type(yuv_bytes))
    
    
    #print(type(yuv_bytes))
    #图像大小参数
    IMG_WIDTH = 504
    IMG_HEIGHT = 378
    IMG_SIZE = int(IMG_WIDTH * IMG_HEIGHT * 3 / 2)
    ...
    #data =b'hello world!'
    #如果传进来的是str，要转为bytes
    #yuv_data = np.frombuffer(bytes(yuv_bytes,encoding = "utf8"), np.uint8)#得到ndarray对象
    ...
    Y, U, V = from_NV12(yuv_data, frames,
                        IMG_WIDTH, IMG_HEIGHT, IMG_SIZE,
                        Y_WIDTH, Y_HEIGHT, Y_SIZE,
                        U_V_WIDTH, U_V_HEIGHT, U_V_SIZE) #获取各通道数据
